...
pub mod pallet {
    ...
    #[pallet::storage]
	pub(super) type Numbers<T: Config> = StorageValue<_, BoundedVec<u32, T::MaxVecLen>, ValueQuery>;
    ...
    impl<T: Config> ValidateUnsigned for Pallet<T> {
        ...
        fn validate_unsigned(source: TransactionSource, call: &Self::Call) -> TransactionValidity {
            ...
            match call {
                ...
                Call::submit_data_unsigned { number: _ } => valid_tx(b"my_unsigned_tx".to_vec()),
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn submit_data(origin: OriginFor<T>, payload: Vec<u8>) -> DispatchResultWithPostInfo {
            ...
            Self::append_or_replace_number(payload[0] as u64);
        ...
        #[pallet::call_index(3)]
        #[pallet::weight({0})]
        pub fn submit_data_unsigned(origin: OriginFor<T>, number: u64) -> DispatchResult {
            ensure_none(origin)?;

            log::info!("OCW ==> in submit_data_unsigned: {:?}", number);
Self::append_or_replace_number(number);
            let key = Self::derived_key(frame_system::Pallet::<T>::block_number());
            let data = IndexingData(b"submit_data_unsigned".to_vec(),number);
            sp_io::offchain_index::set(&key, &data.encode());
        ...
        pub fn unsigned_extrinsic_with_signed_payload(
            ...
            Self::append_or_replace_number(payload.number);
            let key = Self::derived_key(frame_system::Pallet::<T>::block_number());
            let data = IndexingData(b"submit_data_unsigned".to_vec(),payload.number);
            sp_io::offchain_index::set(&key, &data.encode());
            // Return a successful DispatchResultWithPostInfo
            Ok(())
        }
...
impl<T: Config> Pallet<T> {
	...
	fn append_or_replace_number(number: u64) {
		...
		Numbers::<T>::mutate(|numbers| {
			if numbers.try_push(number).is_err() {
				numbers[(number%T::MaxVecLen::get()) as usize]=number;
			}
    ...
    fn send_unsigned_tx(value: u64) -> Result<(), &'static str> {
        ...
        let call = Call::submit_data_unsigned { number: value };
