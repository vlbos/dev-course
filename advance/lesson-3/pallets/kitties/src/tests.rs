...
fn create_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            origin_reserved_balance + <<Test as Config>::StakeAmount as Get<u128>>::get()
        );
...
fn bid_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            origin_reserved_balance + price + <<Test as Config>::StakeAmount as Get<u128>>::get()
        );
        System::assert_has_event(
            Event::<Test>::KittyBid {
                bidder,
...
fn bid_failed_when_bid_for_self() {
    new_test_ext().execute_with(|| {
        let (owner,  kitty_id, price, until_block) = (1, 1, 20, 11);
        assert_ok!(PalletKitties::create(RuntimeOrigin::signed(owner)));
        assert_ok!(PalletKitties::sale(
            RuntimeOrigin::signed(owner),
            kitty_id,
...
fn bid_failed_when_kitty_not_on_sale() {
    new_test_ext().execute_with(|| {
        let (owner, bidder, kitty_id, price) = (1, 2, 1, 20);
        assert_ok!(PalletKitties::create(RuntimeOrigin::signed(owner)));
        // assert_ok!(PalletKitties::sale(
        //     RuntimeOrigin::signed(owner),
        //     kitty_id,
...
fn bid_failed_when_kitty_bid_less_than_the_sum_of_last_price_and_minimum_bid_increment() {
    new_test_ext().execute_with(|| {
        let (owner, bidder,bidder2, kitty_id, price, until_block) = (1,2,4, 1, 20, 11);
        assert_ok!(PalletKitties::create(RuntimeOrigin::signed(owner)));
        assert_ok!(PalletKitties::sale(
            RuntimeOrigin::signed(owner),
            kitty_id,
...
fn bid_failed_when_not_enough_balance_for_bid_and_staking() {
    new_test_ext().execute_with(|| {
        let (owner, bidder, kitty_id, price, until_block) = (1, 4, 1, 20, 11);
        assert_ok!(PalletKitties::create(RuntimeOrigin::signed(owner)));
        assert_ok!(PalletKitties::sale(
            RuntimeOrigin::signed(owner),
            kitty_id,
